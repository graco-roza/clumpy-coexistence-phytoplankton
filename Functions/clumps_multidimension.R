# This code is a modified version of the plot.betadisper function of the package vegan.
#'Here, we included some other aesthetic edits as well as the option @group_2 to add different colour accordind to the MBFGs. 


clump_multi<- function (x, axes = c(1, 2), group_2=group_2, cex = 0.7, pch = seq_len(ng), col = NULL, bg=NULL,
          lty = 3, lwd = 1, hull = TRUE, ellipse = FALSE, conf, 
          segments = TRUE, seg.col = "grey", seg.lty = 2, seg.lwd = lwd, 
          label = TRUE, label.cex = 1, ylab, xlab, main, sub, ...) 
{
  ...
  Ellipse <- function(scrs, centres, conf, col, lty, lwd, ...) {
    ...
    ordiArgAbsorber(xy, FUN = lines, col = col, lty = lty, bg=bg,
                    lwd = lwd, ...)
  }
  if (missing(main)) 
    main <- paste("Functional similarity\n within clumps")
  if (missing(sub)) 
    sub <- paste("")
  if (missing(xlab)) 
  ...
  if (is.matrix(g$centroids)) {
    for (i in seq_along(lev)) {
      ...
      points(g$centroids[j, , drop = FALSE], pch = 22, 
             cex = 1.5, bg = 'black', ...)
    }
  }
  else {
    ...
    points(g$centroids[, 1L], g$centroids[, 2L], pch = 22, 
           cex = 1, bg ='black', ...)
  }
  points(g$sites, pch = pch, cex = cex, col = col[group_2],  bg=bg[group_2],
         ...)
  if (label) {
